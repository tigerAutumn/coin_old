...
public class RedisHelper {
	...
	public SystemCoinType getCoinTypeByCoinSort(Integer coinType) {
		String coins = memCache.get(RedisConstant.COIN_TYPE + coinType);
		if(StringUtils.isEmpty(coins)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(coins);
	...
	public Map<Integer, Integer> getCoinIdToTradeId(Integer agentId) {
		...
		List<SystemTradeType> tradeTypes = getTradeTypeSort(SystemTradeTypeEnum.GSET.getCode(),agentId);
		if (tradeTypes == null) {
	...
	public void set(String key, String t, int seconds) {
		memCache.set(key, t, seconds);
	}
