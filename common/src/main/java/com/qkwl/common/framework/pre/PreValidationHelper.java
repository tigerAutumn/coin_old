...
public class PreValidationHelper {
     ...
     * 组合验证
     */
    public Boolean combinatorialCheck(FUser user) {
    	return !user.isOpenEmailValidate() && !user.isOpenGoogleValidate() && !user.isOpenPhoneValidate();
    }
     ...
     * 新版校验规则
     */
    public Result validateCapitalChack(FUser user, String type) {
        if(validateUserTradePasswordIsSetting(user)){
            return Result.failure(ResultErrorCode.TRADEPASSWORD_SETTING.getCode(), ResultErrorCode.TRADEPASSWORD_SETTING.getValue());
        }
        ...
        if(combinatorialCheck(user)) {
        	return Result.failure(ResultErrorCode.combinatorial_ERROR.getCode(), ResultErrorCode.combinatorial_ERROR.getValue());
        }
