...
public class MQConfig {
    ...
    @Bean(initMethod = "start", destroyMethod = "shutdown")
    public ProducerBean commissionProducer(MQProperties mqProperties) {
        ProducerBean producerBean = new ProducerBean();
        Properties properties = new Properties();
        properties.setProperty("AccessKey", mqProperties.getAccessKey());
        properties.setProperty("SecretKey", mqProperties.getSecretKey());
        properties.setProperty("ONSAddr", mqProperties.getOnsAddr());
        properties.setProperty("ProducerId", mqProperties.getPid().getCommission());
        producerBean.setProperties(properties);
        return producerBean;
    }
    ...
    @Bean
    public CommissionHelper commissionHelper(ProducerBean commissionProducer) {
    	CommissionHelper commissionHelper = new CommissionHelper();
    	commissionHelper.setCommissionHelperProducer(commissionProducer);
