...
import com.qkwl.common.framework.mq.CommissionHelper;
...
public class MatchServiceImpl {
    ...
    public synchronized boolean updateMatch(SystemTradeType systemTradeType, FEntrust buyEntrustWait, FEntrust sellEntrustWait) throws Exception {
        ...
        if (buyEntrust.getFstatus().equals(EntrustStateEnum.AllDeal.getCode()) ||
        	...
        	logger.info("updateMatch retuen 1");
            return false;
        }
...
//        }
        ...
        if (MathUtils.compareTo(buyEntrust.getFprize(), sellEntrust.getFprize()) < 0) {
        	logger.info("updateMatch retuen 2");
...
//		}
        ...
        if (successPrize.compareTo(BigDecimal.ZERO) == -1) {
        	logger.info("updateMatch retuen 3");
        ...
        //佣金
        if ("1".equals(redisHelper.getSystemArgs(ArgsConstant.ISOPENCOMMISSION))) {
        	commissionFee(sellAmountFee,sellEntrust,isSellFee,MathUtils.mul(buyCountFee, successPrize),buyEntrust,isBuyFee);
		}
        ...
        logger.info("updateMatch userCoinWallet end");
    ...
    public synchronized boolean updateMatchHistoryBatch(List<FEntrust> fEntrusts) throws Exception {
        ...
        int result = fEntrustHistoryMapper.insertBatch(historyList);
        if (result <= 0) {
            throw new Exception();
        }
    ...
    //佣金返还
    public void commissionFee(BigDecimal sellAmountFee, FEntrust sellEntrust,boolean isSellFee, 
    		BigDecimal buyCountFee, FEntrust buyEntrust, boolean isBuyFee){
    	//不收手续费的直接返回
    	if (!isSellFee && !isBuyFee) {
    		logger.info("======================不收手续费的无返佣======================");
    		return;
    	}
    	//发送消息队列来执行返佣
    	commissionHelper.SendAmountFee(sellAmountFee, sellEntrust, isSellFee, buyCountFee, buyEntrust, isBuyFee);
    }
