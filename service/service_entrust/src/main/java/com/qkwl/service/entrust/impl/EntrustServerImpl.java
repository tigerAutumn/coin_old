...
public class EntrustServerImpl implements IEntrustServer {
     ...
     */
    @Override
    public Result cancelEntrustBatch(Integer userId, List<String> ids,int type) {
        // 更改订单状态
        try {
        	EntrustDO fEntrust = new EntrustDO();
        	fEntrust.setFstatus(EntrustStateEnum.WAITCancel.getCode());
        	fEntrust.setFuid(userId);
        	fEntrust.setIds(ids);
        	fEntrust.setFtype(type);
            entrustMapper.batchUpdate(fEntrust);
            return Result.success("取消成功");
        } catch (Exception ex) {
            ex.printStackTrace();
            logger.error("cancelEntrust is err, userId:{},entrustId:{}", userId,ids);
        }
        return Result.failure("取消失败");
    }
}