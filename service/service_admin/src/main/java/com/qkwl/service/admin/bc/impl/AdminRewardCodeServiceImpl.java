...
public class AdminRewardCodeServiceImpl implements IAdminRewardCodeService{
	private static final Logger logger = LoggerFactory.getLogger(AdminRewardCodeServiceImpl.class);
	 ...
	 */
	@Override
	public BigDecimal selectWalletTotalAmount(Integer fuid, Integer coinId, Date start, Date end) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("fuid", fuid);
		map.put("type", coinId);
		map.put("start", start);
	 ...
	 */
	@Override
	public Map<Integer,BigDecimal> selectWalletTotalAmount(Integer fuid, Date start, Date end) {
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("fuid", fuid);
			map.put("start", start);
			...
			List<FRewardCodeDTO> totalAmountList = rewardCodeMapper.getTotalAmountList(map);
			return totalAmountList.stream().collect(Collectors.toMap(FRewardCodeDTO::getFtype, FRewardCodeDTO::getFamount));
		} catch (Exception e) {
			logger.error("查询用户兑换总量异常，fuid："+fuid ,e);
			return null;
		}
	}
